...
cmd/                 # Cobra command definitions
  ...
  removed/           # hidden guidance stubs for removed commands (one release, #477)
...
- **Name resolution**: a `GetXIDByName` that leans on a server-side exact-match filter goes through `api.ResolveByName[T]` (`api/resolve.go`), never its own body. A name that trims to empty is refused before any request, because the server drops a filter whose value is empty and answers with the whole list—`Results[0]` then resolves to an arbitrary resource, which is how a blank `--server` once granted a token ACL on someone else's server (#436). The helper is the one place that trims, refuses, fetches a single page under the caller's filter key, and decides not-found on `len(results)` rather than `Count`, which a positive `Count` with an empty `Results` would have panicked on. Its options struct names each field so two same-typed arguments cannot be swapped silently. The refusal carries `api.ErrBlankName`, so a caller tells it apart with `errors.Is` instead of matching message text—`cmd/iam`'s transposed-argument hint skips itself on it, and `api/rbac` re-exports the sentinel so `cmd/` need not import `api`. `api.RequireName` is the trim-and-refuse half on its own, for the one resolver that cannot use the helper: `api/packages`'s entries endpoints filter on `package__name` and not `name`, so `GetPackageIDByName` narrows with `search` and compares `Name` exactly on the client side
...
- **Config file**: `~/.alpacon/config.json` (dir `0700`, file `0600`). `saveConfig` writes a sibling temp file and renames it over the target—two alpacon processes can be saving a renewed access token at the same moment, and a plain truncate-and-write leaves a config the next command cannot parse. Any new writer must go through `saveConfig`, which resolves a symlinked `config.json` before the rename so a config kept in a dotfiles tree survives the write, as `DeleteConfig` does before the remove. Anything already wider than those modes is narrowed in place before it is read or written: `config/permissions.go` decides what to narrow and states the whole rationale in its package comment—read that before changing anything here—`config/permissions_unix.go` holds the guards it refuses on, and the platform-specific opens live in `config/permissions_darwin.go` and `config/permissions_linux.go`. Two refusals are not best effort and carry `errUnsafeConfigFile`: a config file that is not a regular file, and one owned by another account. Every other narrowing failure warns once and the command carries on, except `~/.alpacon/device_id`, which fails closed. A symbolic link is the one failure that reads the target before it speaks: the chmod is refused on the link rather than on what it names, so the warning is kept back unless the target's own mode grants another account access—otherwise a config kept in a dotfiles tree draws a notice on every command for an exposure that is not there. All of it is Unix only—Windows chmod changes the read-only attribute, so nothing is repaired there and the device id is not protected either.
- **Alias**: `alpacon` can also be invoked as `ac`
- **File transfer**: The `cp` command lives in `cmd/ftp/` (package name `ftp`)
- **Exit codes**: `0` success, `1` general error, `2` usage error (only `work-session`, `event wait`/`watch`, and `utils.RequirePositiveInt`; every other command's own validation and Cobra parse errors still exit `1`), `3` WorkSession gate denied, `4` pending approval with the outcome still open, `5` server busy, `6` approval not granted, `7` purpose required—an agent's command held while the gate asks what it is for, answerable by the caller rather than by an approver, `8` a newer release exists (only `alpacon update --check`), `9` a removed command's guidance stub ran (for one release; see #477) (constants in `utils/error.go`). Keep these stable—scripts, CI, and AI agents branch on them. See README "Exit codes".
