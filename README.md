...
Exit code is `1`, and under `--output json` the refusal is an error envelope on stderr with `error_code` `command_inline_credential`. Its `next_actions` contains a `command` example using `--env="SECRET_NAME"`, matching the invocation: `exec` takes the remote command after `--`, and `websh` takes it as one quoted argument. Replace the example's secret name, server, and command with your own values. The example never includes the rejected command line or any credential value.
