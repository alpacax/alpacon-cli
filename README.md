...
$ alpacon work-session extend <session-id> --expires-in 2h
$ alpacon work-session extend <session-id> --expires-in 2h --reason "customer escalation, still triaging" --wait
...
Some workspaces gate `extend` behind approval. When that is on, `--reason` is required—a short justification an approver judges the request by—and the session may not extend immediately: the CLI reports the extension request pending (`status` `pending_approval` under `--output json`, exit `4`) instead of the session already extended. Pass `--wait` (or `--wait-approval <duration>` for a longer timeout, default 5m) to block until the request is decided. `error_code` `work_session_extension_reason_required` means the workspace requires `--reason`; `work_session_extension_already_pending` means a decision is already outstanding for this session—wait for it (or cancel it with `alpacon approval cancel <request-id>`) before asking again.
...
| `6`  | Approval not granted—an awaited approval settled without being granted (rejected, expired, revoked, cancelled, or completed). Distinct from `4`: the outcome is final, so retrying the same request only generates another approval request. Returned by `alpacon event wait`, by `work-session create --wait`, by `work-session extend --wait` when the extension request is rejected, cancelled, or expires before a decision, and—when the command's own approval was refused or its window lapsed—by `exec`, by `websh` command mode, and by `alpacon exec logs`. Under `--output json` these paths emit an error envelope carrying the same `exit_code`. For `work-session create --wait` and `work-session extend --wait`, the session already exists and outlives the refusal, so the CLI names `work-session describe <id>` and `work-session complete <id>` as next steps—not `work-session use <id>`, which only attaches an active session and a settled one can never become active again—on stderr as plain text, and under `--output json` both as `next_actions` and as `work_session_id` in the envelope's `context` |
