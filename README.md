...
| `2`  | Usage error—a flag or argument rejected by a `work-session` subcommand, by `event wait` / `event watch`, or by the positive-value check behind `--tail`, `--limit`, and `--valid-days` (`utils.ExitCodeUsageError`). Under `--output json` the envelope carries `error_code` `usage_error`. Every other command still exits `1` when it rejects a flag or argument, as do errors Cobra itself rejects while parsing (an unknown flag or subcommand) |
