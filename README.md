...
For this release only, the removed commands (`server reboot`, `server shutdown`, `server upgrade`, `agent shutdown`) are hidden stubs: they print what to run instead and exit with code `9`, without contacting the server.
...
| `5`  | Server busy with active user work—`alpacon agent restart` or `alpacon agent upgrade` was refused because the server has an open Websh/WebFTP session or in-flight command. Transient and retryable: retry when idle, or re-run with `--force` to override |
...
| `9`  | Command removed—`alpacon server reboot`, `server shutdown`, `server upgrade`, or `agent shutdown` was run. These commands were removed; for this release they print what to run instead (`alpacon exec` in a work session, or `alpacon agent restart`) and exit `9` without contacting the server. Distinct from `1` so a script can tell a command that no longer exists from one that failed |
