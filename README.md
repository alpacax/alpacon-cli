...
| `4`  | Pending human approval—the action is awaiting an out-of-band approve/reject in the Alpacon console (web/Slack), not refused. For `exec` and `websh SERVER COMMAND` (non-interactive execution), re-run the command after approval (or pass `--wait` on `exec` to block); for `work-session create` the session already exists—after approval attach it with `alpacon work-session use <id>` (or pass `--wait` on the original create to block). Under `--output json`, a `{"status":"pending_approval", ...}` object is emitted. Returned by `exec` and `websh SERVER COMMAND` on a `SUDO_APPROVAL_REQUIRED` sudo denial and by `work-session create` when the session lands pending. `alpacon exec logs JOB_ID --output json` also reports a held job as `{"status":"pending_approval", ...}`, keeping exit code 0 for that lookup |
| `5`  | Rejected by a reviewer—a human denied the action out of band in the Alpacon console (web/Slack); the command will not run, do not retry it. Under `--output json`, a `{"status":"rejected", "error_code":"command_rejected", ...}` object is emitted. Returned by `exec`, `websh SERVER COMMAND`, and `exec logs` when the underlying command's status is `rejected` |
