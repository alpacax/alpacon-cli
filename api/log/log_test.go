...
func TestGetSystemLogList_InvalidPageSize(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
	...
	_, err := GetSystemLogList(ac, "test-server", 0)
	if err != nil {
		t.Fatalf("GetSystemLogList error with pageSize=0: %v", err)
	}

	_, err = GetSystemLogList(ac, "test-server", -10)
	if err != nil {
		t.Fatalf("GetSystemLogList error with pageSize=-10: %v", err)
	}
}
