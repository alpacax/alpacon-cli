...
- The `goreleaser` job attests every archive and package it builds via `actions/attest-build-provenance`, which is why it holds `id-token: write` and `attestations: write`. Do not drop either permission
